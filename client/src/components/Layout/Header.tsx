...
const Header: React.FC = () => {
  ...
  useEffect(() => {
    if (showAppMenu && appButtonRef.current) {
      ...
      const dropdownWidth = 320; // Updated width for new design
      const viewportWidth = window.innerWidth;
      
      // Calculate position to keep dropdown in viewport
      let leftPos = rect.right - dropdownWidth;
      if (leftPos < 16) { // 16px padding from left edge
        leftPos = 16;
      }
  ...
  return (
    <header className="bg-white/95 backdrop-blur-sm shadow-sm border-b border-gray-200 fixed top-0 left-0 right-0 z-50" style={{ isolation: 'isolate' }}>
            ...
            <nav className="hidden lg:flex items-center space-x-6">
              ...
              <div className="relative">
                ...
                {showMoreMenu && (
                  <div 
                    ref={moreMenuRef}
                    className="absolute left-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-200 py-2 z-[9999999]"
                    style={{
                      position: 'fixed',
